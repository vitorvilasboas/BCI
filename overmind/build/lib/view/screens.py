...
class GalaxyMenu(Screen):
    ...
    def clock_scheduler(self):
        ...
        Clock.schedule_interval(self.get_probs, 1. / 20.)
        Clock.schedule_interval(self.update_command, 1./20.)
        # Clock.schedule_interval(self.update_accum_bars, self.session.control.window_overlap)
        if self.session.acq.mode == 'simu': # and not self.session.acq.dummy and not self.session.acq.path_to_labels_file == '':
            Clock.schedule_interval(self.update_current_label, 1. / 20.)
...
class GalaxyPlay(threading.Thread):
    ...
    def define_pygame(self):
        ...
        self.inst = pygame.image.load("tests/gametest/instruc2.png")
        self.spacet = pygame.mixer.music.load('tests/gametest/spacet.ogg')
        # pygame.mixer.music.play()
        self.startim = pygame.font.Font('freesansbold.ttf', 32)

        # nave
        self.nave = pygame.image.load("tests/gametest/nave.png")
        self.x = 346  # largura
        self.y = 480  # altura
        self.move = pygame.mixer.Sound('tests/gametest/woosh.ogg')
        self.ded = pygame.mixer.Sound('tests/gametest/ded.ogg')
        ...
        # asteroid
        self.asteroidim = pygame.image.load("tests/gametest/asteroid3.png")
        ##self.asteroidx = random.randint(0,5)
        ##self.asteroidy = random.randint(-150,-130)
        self.asteroidx = random.choice([0, 350])
        self.asteroidy = -330 # -130
        self.astemusic = pygame.mixer.Sound('tests/gametest/asteroid.ogg')
    ...
    def run(self):
        ...
        self.space = pygame.image.load("tests/gametest/space2.png")
        ...
        while not self.sair: #and not self.stop_flag:
            ...
            if self.game_over == False and self.play == True:
                ...
                if self.timev2 >= 3: self.asteroidy += 1
                ...
                elif self.x >= 670: self.x = 670

                # asteroid_mechanics
                if self.asteroidy >= 550:
                    self.colisoes += 1
                    self.asteroidx = random.choice([0, 350])
                    self.astemusic.play()
                    self.asteroidy = -330 # -130
                ...
                if self.distancey <= 330 and -9 <= self.distancex <= 389:
                    self.nave = pygame.image.load("tests/gametest/explosion.png")
