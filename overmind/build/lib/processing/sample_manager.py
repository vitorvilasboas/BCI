...
class SampleManager(threading.Thread): # define a classe manager
    def __init__(self, srate, p, buf_len=None, tmin=0.5, tmax=2.5, class_ids=[1,2], daisy=False, mode='openbci', path=None, labels_path=None, dummy=False):
        ...
        if self.acq_mode == 'openbci':
            ...
            if self.dummy:
                ...
                if not self.playback_labels == []:
                    # self.playback_labels = iter(load_npy_data(self.playback_labels_path))
                    self.playback_labels = iter(self.playback_labels)
    ...
    def append_epoch(self):
        ...
        if self.current_cmd == 0:  # usado quando o rótulo da classe NÃO é passado
            idx1 = np.where(self.labels == self.class_ids[0])[0]
            idx2 = np.where(self.labels == self.class_ids[1])[0]
            # print(idx1.shape, idx2.shape)
            if self.winning == 1:
                k = randint(0, len(idx2) - 1)  # escolhe uma amostra aleatoria da classe B
                idx = idx2[k]
            else:
                k = randint(0, len(idx1) - 1)  # escolhe uma amostra aleatoria da classe A
                idx = idx1[k]
        else:
            idx12 = np.where(self.labels == self.current_cmd)[0]  # usado quando o rótulo da classe é passado
            k = randint(0, len(idx12) - 1)
            idx = idx12[k]
        self.playbackData = np.vstack([self.playbackData, self.epochs[idx].T])
