...
if __name__ == "__main__":
    ...
    subjects = [1] # range(1,55) 
    classes = [[1, 2]]

    R = pd.DataFrame(columns=['subj','A','B','as_best','sb_iir','cla_iir'])
    
    FINAL = pd.DataFrame(columns=['subj', 'A', 'B', 'as_max_cal', 'as_tune_cal', 'as_max', 'as_tune', 'as_mode', 'as_pmean', 'as_best', 'sb_iir', 'cla_iir', 
                                  'as_on', 'sb_iir_on', 'cla_iir_on', 'as_com_acum', 'as_com_mode', 'as_com_pmean', 'sb_iir_com_acum', 'sb_iir_com_mode', 'sb_iir_com_pmean',
                                  'cla_iir_com_acum', 'cla_iir_com_mode', 'cla_iir_com_pmean', 'fl', 'fh', 'tmin', 'tmax', 'nbands', 'ncsp', 'csp_list', 'clf', 'clf_details', 
                                  'as_acum_ncom', 'as_acum_ncorrect', 'as_mode_ncom', 'as_pmean_ncom', 'sb_iir_acum_ncom', 'sb_iir_acum_ncorrect', 'sb_iir_mode_ncom', 
                                  'sb_iir_pmean_ncom', 'cla_iir_acum_ncom', 'cla_iir_acum_ncorrect', 'cla_iir_mode_ncom', 'cla_iir_pmean_ncom', 'as_route', 'as_route_ok',
                                  'sb_iir_route', 'sb_iir_route_ok','cla_iir_route', 'cla_iir_route_ok'])
    
    for suj in subjects:
        for class_ids in classes:  
            H = H.iloc[0:0] # cleaning df
            
            data, events, info = np.load('/mnt/dados/eeg_data/Lee19/npy/S' + str(suj) + '.npy', allow_pickle=True)
            data_cal, events_cal, _ = np.load('/mnt/dados/eeg_data/Lee19/npy/S' + str(suj) + 'sess1.npy', allow_pickle=True)
            data_val, events_val, _ = np.load('/mnt/dados/eeg_data/Lee19/npy/S' + str(suj) + 'sess2.npy', allow_pickle=True)
            Fs = info['fs']
            filtering = 'DFT'
            ...
            space = (
                ...
                hp.choice('ncomp', [2,4,6,8,22,40,60]),
            ...
            ncsp = [2,4,6,8,22,40,60][best['ncomp'][0]] # int(best['ncomp'][0])
            tmin = best['tmin'][0]
            tmax = best['tmax'][0]
            ...
            acc_max, _ = sbcsp_approach(ZT, ZV, tt, tv, nbands, fl, fh, ncsp, clf, Fs=Fs, filt=filtering) 
            
            ##%% #########################################################################
            ZT, ZV, tt, tv = half_split_data(data, events, int(tmin*Fs), int(tmax*Fs), class_ids)
            acc_tune, _ = sbcsp_approach(ZT, ZV, tt, tv, nbands, fl, fh, ncsp, clf, Fs=Fs, filt=filtering, csp_list=csp_list)     
            ...
            acc_pmean = np.mean(pv == tv)
            
            acc_best = max(acc_tune, acc_max, acc_mode, acc_pmean) 
            ...
            ##%% ###########################################################################
            print(f" Max: {round(acc_max*100,2)} | {acc_train} >> {fl}-{fh}Hz; {tmin}-{tmax}s; Ns={nbands}; R={ncsp}; {clf}")
            print(f"Tune: {round(acc_tune*100,2)} | {acc_train_tune} >> {csp_list}")
            print(f"Moda: {round(acc_mode*100,2)} | Media: {round(acc_pmean*100,2)}")
            print(f"  AS: {round(acc_best*100,2)} |    SB: {round(sb_iir*100,2)} |   CLA: {round(cla_iir*100,2)}\n")
            
            R.loc[len(R)] = [suj, class_ids[0], class_ids[1], acc_max, acc_train, acc_best, sb_iir, cla_iir]
            ...
            as_online, LA = sbcsp_approach(ZT, ZV, tt, tv, nbands, fl, fh, ncsp, clf, Fs=info['fs'], filt='DFT')
            # as_online, y, p = tester(ZV, tv, Fs, class_ids, hmax) # (TREINO SOMENTE E)
            print(f"ASon : {round(as_online*100,2)}")    
            ...
            while (i < len(y)): # for i in range(nta, len(y), 1):
                ...
                if (U1_prop >= limiar_gatilho): #  and (tv[i]==class_ids[0])
                    command_list.append(np.array([class_ids[0], i])) # [ comando, momento do envio (indice época)]
                ...
                if (U2_prop >= limiar_gatilho): #  and (tv[i]==class_ids[1])
                    command_list.append(np.array([class_ids[1], i]))
            ...
            for i in range(len(command_list)): 
                if command_list[i,0] == tv[command_list[i,1]]: corretos_cont += 1; corretos_list += '1 '
            ...
            sb_iir_online, LS = sbcsp_approach(ZT, ZV, tt, tv, 9, 4, 40, 8, {'model':'SVM','kernel':{'kf':'linear'},'C':-4}, Fs=info['fs'], filt='IIR')
            print(f"SBon : {round(sb_iir_online*100,2)}");
            ...
            ZT, _, tt, _ = half_split_data(data, events, smin, smax, class_ids)
            cla_iir_online, LC = classic_approach(ZT, ZV, tt, tv, 8, 30, 8, {'model':'LDA'}, Fs=info['fs'], filt='IIR')
            print(f"CLAon: {round(cla_iir_online*100,2)}")
            ...
            for i in range(len(command_list)): 
                if command_list[i,0] == tv[command_list[i,1]]: corretos_cont_cla_iir += 1; corretos_list_cla_iir += '1 '
                else: corretos_list_cla_iir += '0 '
            
            # taxa de acerto de comando == comando enviado / comando esperado : corretos_cont/len(command_list) ou np.mean(command_list[:, 0] == tv[command_list[:,1]])
            # print(no_asteroidx, cont_com_a, cont_com_b, cont_no_com)
            acc_cla_iir_acum = round(np.mean(command_list[:, 0] == tv[command_list[:,1]])*100,2)
            ...
            FINAL.loc[len(FINAL)] = [suj, class_ids[0], class_ids[1], acc_train, acc_train_tune, acc_max, acc_tune, acc_mode, acc_pmean, acc_best, sb_iir, cla_iir, 
                                  as_online, sb_iir_online, cla_iir_online, 'as_com_acum', 'as_com_mode', 'as_com_pmean', 'sb_iir_com_acum', 'sb_iir_com_mode', 'sb_iir_com_pmean',
                                  'cla_iir_com_acum', 'cla_iir_com_mode', 'cla_iir_com_pmean', fl, fh, tmin, tmax, nbands, ncsp, csp_list, clf['model'], clf, 
                                  'as_acum_ncom', 'as_acum_ncorrect', 'as_mode_ncom', 'as_pmean_ncom', 'sb_iir_acum_ncom', 'sb_iir_acum_ncorrect', 'sb_iir_mode_ncom', 
                                  'sb_iir_pmean_ncom', 'cla_iir_acum_ncom', corretos_cont_cla_iir, 'cla_iir_mode_ncom', 'cla_iir_pmean_ncom', 'as_route', 'as_route_ok',
                                  'sb_iir_route', 'sb_iir_route_ok','cla_iir_route', corretos_list_cla_iir]
