...
if __name__ == "__main__":
    ...
    for suj in subjects:
        for class_ids in classes:  
            ...
            data_cal, events_cal, _ = np.load('/mnt/dados/eeg_data/IV2a/npy/A0' + str(suj) + 'T.npy', allow_pickle=True)
            data_val, events_val, _ = np.load('/mnt/dados/eeg_data/IV2a/npy/A0' + str(suj) + 'E.npy', allow_pickle=True)
            Fs = info['fs']
            filtering = 'DFT'
            ...
            print(f"Tune: {round(acc_tune*100,2)} | Moda: {round(acc_mode*100,2)} | Media: {round(acc_pmean*100,2)}")
            ...
            sb_online, LS = sbcsp_approach(ZT, ZV, tt, tv, 9, 4, 40, 8, {'model':'SVM','kernel':{'kf':'linear'},'C':-4}, Fs=info['fs'], filt='IIR')
            print(f"SBon : {round(sb_online*100,2)}");
            ...
            ZT, _, tt, _ = half_split_data(data, events, smin, smax, class_ids)
            cla_online, LC = classic_approach(ZT, ZV, tt, tv, 8, 30, 8, {'model':'LDA'}, Fs=info['fs'], filt='IIR')
            print(f"CLAon: {round(cla_online*100,2)}")
