...
if __name__ == "__main__":
    ...
    for suj in subjects:
        for class_ids in classes:  
            ...
            ##%% ############################################################################                                                         
            #%%
            delta_t = 0.1               # deslocamento em segundos
            ...
            n_rounds = 1        # número de partidas jogadas
            ...
            SA, SB = trials_a[np.random.randint(0, len(trials_a)-1)], trials_b[np.random.randint(0, len(trials_b)-1)]
            for i in range(1, len(trials_a)): SA = np.c_[ SA, trials_a[np.random.randint(0, len(trials_a)-1)] ] # trials_a[i]
            for i in range(1, len(trials_b)): SB = np.c_[ SB, trials_b[np.random.randint(0, len(trials_b)-1)] ] # trials_b[i]
            ...
            # buffers_a1 = np.asarray([ SA[:, i:i+q] for i in range(0, SA.shape[-1], q) ])
            # buffers_b2 = np.asarray([ SB[:, i:i+q] for i in range(0, SB.shape[-1], q) ])
        
            ZV, tv = [], [] # acumulador de buffers/épocas e respectivos rótulos para validação
            cont_a, cont_b = 0, 0 # contadores de buffers/épocas A, B adicionadas ao acumulador para validação
            for i in range(n_rounds): 
                if safe_targets[i] == class_ids[0]:
                    ZV.append(buffers_a[cont_a : cont_a + n_epochs]) # add buffers/épocas da classe A no acumulador
                    cont_a += n_epochs # incrementa o contador de buffers/épocas A usadas
                else:
                    ZV.append(buffers_b[cont_b : cont_b + n_epochs]) # add buffers/épocas da classe A no acumulador
                    cont_b += n_epochs # incrementa o contador de buffers/épocas A usadas
                tv.append(safe_targets[i] * np.ones(n_epochs))
            ...
            as_on, learner = sbcsp_approach(ZT, ZV, tt, tv, nbands, fl, fh, ncsp, clf, Fs=info['fs'], filt='DFT')
            ...
            coord_ast = screen_size_x*0.25 if (safe_targets[0] == class_ids[1]) else screen_size_x*0.75 
            ...
            distancia_final = abs(coord_ast - coord_nave)
            
            max_comands_possiveis = 0; i = nta
            while (i < len(y)): max_comands_possiveis += 1; i += int(1/delta_t)
            ...
            if not as_comlist == []:
                ...
                as_command = round(np.mean(as_comlist[:,0] == tv[as_comlist[:,1]])*100,2) # list(as_comlist[:, 2]).count(1)/len(as_comlist), tx acerto comando == com enviado/com esperado, corretos/len(as_comlist)
            else: as_command = 0.0; as_comlist = None
            ...
            print(no_asteroidx)
            print('escapou') if distancia_final > 200  else print('morreu')
            if not as_comlist is None: print(as_comlist[:,1])
